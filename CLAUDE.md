...
- **Phase 4 — AI paste-parse / macro-estimate**: `ai-proxy` gained a second call mode alongside the existing conversational tool-calling loop — passing `responseSchema` in the request body routes into `callGeminiStructured`, a single-shot Gemini call with `responseMimeType: 'application/json'` + a Gemini-format JSON schema, returning `{ data }` instead of `{ text }` (no tools, no multi-turn loop; used for structured extraction, not chat). `src/features/ai/api/aiApi.ts` exposes `parseRecipeText(text)` and `estimateRecipeMacros(ingredients, servings)` on top of this. In `RecipeModal`: "✨ Paste recipe" reveals a textarea — parsing fills title/servings/instructions/ingredients + a macro estimate (mode forced to `manual`, still fully editable, nothing auto-saved). "✨ Estimate with AI" (manual macro mode only) fills the five macro fields from the current ingredient rows. Both are one-shot fills the user reviews before saving — never call these tools from the general Ask AI chat panel, they're editor-only actions.
