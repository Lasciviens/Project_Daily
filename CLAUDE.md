...
- **`HealthTab.tsx` UI** (Apple Health-inspired, own palette — not a copy): a pill-navigated set of sections — **Overview** (hand-rolled SVG `ActivityRings` for Move/Exercise/Stand, inspired by Apple's activity rings, plus the workouts list), **Steps** (today's count + hourly bar chart + distance/pace), **Energy** (active+basal stacked area over 14 days), **Heart** (daily min/max/avg range + resting HR + HRV), **Sleep** (last-night stage breakdown bar + 14-day total-sleep trend), **Body** (weight/body-fat/BMI, sparse-point line+bar combo over 90 days), and **All Data** (the original generic filterable table, kept as a catch-all for the ~40 metrics that don't have a dedicated section). Charts use `recharts` (first chart-library dependency in the project); rings are custom SVG since no library has an "Apple-style concentric rings" component.
