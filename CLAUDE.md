...
- **`HealthTab.tsx` UI** (Apple Health-inspired, own palette — not a copy): a pill-navigated set of sections — **Overview** (hand-rolled SVG `ActivityRings` for Move/Exercise/Stand + a collapsed-by-default workouts list), **Steps** (hourly count + distance/pace), **Energy** (active+basal stacked bars), **Heart** (daily min/max/avg range + resting HR + HRV), **Sleep** (last-night stage breakdown + trend), **Body** (weight/body-fat/BMI), and **All Data** (the original generic filterable table, kept as a catch-all for the ~40 metrics that don't have a dedicated section). Steps/Energy/Heart share a `PeriodToggle` (Day/Week/Month, default Week — Day is hourly, Week/Month are daily bars over the last 7/30 days); Sleep has a Week/Month toggle for its trend chart only. Charts use `recharts` (first chart-library dependency in the project); rings are custom SVG since no library has an "Apple-style concentric rings" component. **Canonical chart style** (user-preferred, established with Body, reused for Heart — default for any future Health chart): a translucent `Bar` + a `Line` with dots on top, same color, via the shared `BarLineChart.tsx` component (optional `rangeKey` prop adds a faint min–max band behind it, used by Heart). `TrainingPage`'s right rail shows `TrainingCalendar` for Hevy/Strava but swaps to `HealthStatsPanel` (short, plain-computed — **no AI** — per-section analysis: averages, best/worst day, trend vs. the previous week) when the Health tab is active, since the calendar isn't relevant there.
