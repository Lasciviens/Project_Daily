...
- **Muscles sub-tab** (`WorkedMuscles.tsx`): a clean stylised front/back body (`react-muscle-highlighter`, MIT) on a dark panel, heat-mapped by which muscle groups your logged workouts hit over a **Day / Week / Month** period (selector). Body is ALWAYS shown (neutral grey when nothing's logged). `hevyApi.fetchWorkoutExerciseTemplateIds(from,to)` flattens the range's `hevy_workout_exercises.exercise_template_id`s; each id → `primary_muscle_group` (via `useHevyExerciseTemplates`) → a body **slug** through `muscleMap.ts`'s `HEVY_TO_SLUG`, counted and banded into a 5-step teal→red intensity ramp (`colors` prop; `defaultFill` grey base). Front/Back toggle, intensity legend, top-worked chips, and **tap a muscle (or chip) → detail card listing the distinct exercises that hit it this period + how many times each**. **`HEVY_TO_SLUG` covers the COMPLETE Hevy `primary_muscle_group` enum** (20 values, verified against Hevy's OpenAPI `MuscleGroup` schema: abdominals→abs, shoulders→deltoids, lats/upper_back→upper-back, traps→trapezius, glutes/abductors→gluteal, adductors, quadriceps, hamstrings, calves, biceps, triceps, forearms→forearm, lower_back→lower-back, chest, neck; cardio/full_body/other→null, not a single muscle) — so **every** logged exercise maps to the right region. (Replaced the earlier `react-body-highlighter` cartoon SVG and a rejected `react-three-fiber` 3D-écorché attempt — the 3D BodyExplorer model rendered but looked too "medical"; the user chose this stylised 2D. Full history + the BodyExplorer GLB pipeline notes are in the git log of branch `claude/charming-newton-yhk8i`.)
