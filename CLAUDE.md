...
- **Routine set payloads**: Hevy rejects `rep_range: null` (400 "must be of type object") — send `rep_range: {start,end}` only when a range is entered, otherwise OMIT the key and send fixed `reps`. `RoutineModals.formToPayload` builds sets this way; `hevy-api` also strips any null `rep_range` defensively before POST/PUT.
