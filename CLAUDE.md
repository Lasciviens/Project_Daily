...
- **Exercise demo images** (`exerciseMedia.tsx`, wired into `ExerciseTemplatesTab`'s `TemplateCard` via `ExerciseThumb`): Hevy's API returns no exercise media, so images come from **free-exercise-db** (Unlicense, jsDelivr CDN, CORS-open) — `useExerciseImageDb` loads the catalog once, `matchExerciseImage` fuzzy-matches by normalized name tokens, graceful no-image fallback. Test-grade (name-match, not curated).
...
**Not done yet:**
- 🔶 **Exercise demo media — polish pass** (test shipped): free-exercise-db name-match images are live in `ExerciseTemplatesTab` (see Training Feature Detail). Follow-up if the match rate is poor: better fuzzy matching / a manual override map, or swap to a gif source (ExerciseDB/WorkoutX) for animated demos. Same fuzzy-match layer could feed the Muscles sub-tab and workout detail too.
