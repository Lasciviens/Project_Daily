...
**Health Auto Export (v1, migration `036_health_export.sql`)**: Huawei's own Health Kit API is closed to individual developers (requires AppGallery-published app), so instead: Huawei Health on iOS already syncs into Apple HealthKit; the **Health Auto Export** app (App Store) reads HealthKit and POSTs to `health-export-webhook` on a schedule. `health_workouts` (comparable shape to `hevy_workouts` — id/name/start/end/duration/heart rate/energy, plus a `raw` jsonb with the full payload incl. per-minute arrays) and `health_metrics` (generic per-metric-per-day rows — `value` jsonb since shape varies: `qty` vs `Min/Avg/Max` vs multi-field sleep) are separate from Hevy's tables (shared `HEVY_USER_ID`, different source). Shown as its own top-level **Health** pill tab on `TrainingPage` (sibling to Hevy/Strava, not nested under Hevy) — `HealthTab.tsx` is a plain browse view: recent workouts as a list, and all metrics in a filterable table (metric name / source / last-N-days dropdowns, client-side over a bounded fetch — this is a personal single-user dataset, no need for server-side filtering or pagination yet). Not yet merged into `TrainingCalendar`'s calendar view or charted; that's a deliberate follow-up once the pipeline is confirmed reliable. Importable Health Auto Export automation configs (daily/weekly-reconciliation/workouts/one-time-backfill) live in `docs/health-auto-export/`. Reminder from Apple's own platform docs: HealthKit is unreadable while the phone is locked and background task scheduling is opportunistic — so delivery is best-effort, not guaranteed-real-time, by OS design (not a bug in either app). **App must be set to "Export Version 2"** in Health Auto Export's Workout Configuration (current format — v1 is legacy, no `id` field, different field shapes). If a workout ever arrives without `id` anyway, the webhook falls back to a deterministic key (`name+start+end`) instead of dropping it — still idempotent on re-delivery, just not a real UUID; only a workout with no id AND no name/time to build that key from is actually skipped (counted in the response's `skipped_workouts`). Known gap: `stateOfMind`/`medications`/`symptoms`/`cycleTracking`/`ecg`/`heartRateNotifications` top-level payload arrays are not ingested (not present in the real export sample tested against; add if/when needed). Workout numeric fields (`duration`, `activeEnergyBurned`, `totalEnergy`, and each of `heartRate.avg/min/max`) arrive as either a plain number or a `{qty, units}` quantity object depending on the field — confirmed against real production payloads, not just the one test sample — so the webhook unwraps either shape (`numOrQty()`) rather than assuming one.
...
| Function | Purpose |
|---|---|
| `ai-proxy` | Gemini AI calls. **Generic DB layer** (`describe_database`, `db_query`, `db_insert`, `db_update`, `db_delete`) over a curated `DB_CATALOG` allow-list — the AI reads/writes any of the user's own tables (tasks, time_blocks, recipes+ingredients+meal_plans, shop_*, projects, media entries). Guardrails: default-deny allow-list (token/secret/auth tables unreachable), `access: 'rw'|'ro'` per table (hevy_*/strava_activities/health_daily_stats/movies/tv_series are read-only — synced externally), every op force-scoped to `user_id`, no DDL (structured ops, not raw SQL), update/delete refuse empty filters. Reads `app_error_logs` (read-only) too, so the AI can help diagnose failures. **Behavioral guardrails in the system prompt**: the AI must get explicit user confirmation BEFORE any delete (never auto-deletes), and must announce what it's about to do before any create/update/delete. Plus special-purpose tools that generic CRUD can't cover: `get_calendar_events`/`get_next_transit` (external APIs), `get_media`/`plan_media`/`mark_episode_watched` (media logic), `get_health_stats` (weekly avgs), `get_shop_categories`/`create_shop_category`/`create_shop_item`/`ask_clarifying_question` (Shop chat panel). **Transit toolkit** (5 tools, not just one) — built after transit answers going wrong was flagged as a recurring, explicit pain point, so the AI gets room to investigate instead of one opaque black-box call: `get_saved_transit` (list the user's own saved stops/routes + labels, read-only — call this first when "home"/"work"/an ambiguous place is unclear), `search_transit_stops` (free-text search over ANY stop/address via the EnTur geocoder, returns exact ids — use to disambiguate or get precision), `save_transit_stop`/`save_transit_route` (persist a stop/route found via search as a labeled favorite — plain inserts into `user_transit_stops`/`user_transit_routes`, mirrors the client's `useTransitStops`/`useTransitRoutes` hooks exactly, e.g. first saved stop auto-becomes default), `get_next_transit` (departures from a saved-by-name stop OR any stop by exact id), and **`plan_trip`** (point-to-point EnTur journey planning with transfers — the gap `get_next_transit` alone always had). Place resolution in `plan_trip`/`get_next_transit` is deterministic server-side code, never left to the model: an exact `NSR:StopPlace:` id skips straight through (with a small extra GraphQL call to resolve its real name for narration); empty → user's default saved stop; "home"/"ev"/"work"/"iş" (+ variants) fuzzy-match the user's own saved labels; otherwise falls through to the geocoder. Only a regex-validated `NSR:StopPlace:` id or finite numeric coordinates are ever interpolated into a GraphQL query (injection-safety, consistent across all these functions). System prompt has explicit anti-hallucination rules: never invent stops/lines/times, verify via `get_saved_transit`/`search_transit_stops` before guessing, retry with a found exact id before giving up, report tool failures plainly. New GraphQL fields (e.g. mode filters, service disruption/situations data) were deliberately NOT added speculatively — the schema wasn't independently verifiable in-session (EnTur's API + developer docs were both blocked by this sandbox's own network egress policy, not by EnTur), so guessing field names was judged riskier than leaving them out; add them once verified against real EnTur docs. Also `responseSchema` (structured recipe extraction) + `fetchUrl` (server-side page fetch) modes. Dead `log_workout`/`get_workouts` removed (hit the dropped `training_sessions` table — training is now Hevy/Strava, read via `db_query`; planned sessions = `time_blocks` category='training'). |
