...
| 4 | UI + AI wiring (source switch, Sleep, mini-cards, coach) | 🟠 **CORE SHIPPED 2026-07-21 (same day as Phase 3)** — per-section Auto/Apple/Google source switch (Steps/Energy/Heart/Sleep; 'Auto' = resolver union, others = pure single-family via fetch filter — the CARDINAL RULE's on-demand-source UI), FitbitHypnogram (renders from health_sleep_segments the moment the first night lands; wake/REM/light/deep lanes + per-stage totals), mini-cards registered for oxygen_saturation/skin_temperature/sleeping_heart_rate (Sleep) + active_zone_minutes (Energy). AI source-awareness shipped earlier (bf89990) + redeployed by user. REMAINING for post-vacation: Body-section toggle (different header layout), Source Lab / compare view, hypnogram verification against the first real night, v1.1 poller metrics. | Yes (real data to show) | Phase 3 producing real rows |
...
- [x] `get_health_stats` source-aware **in code** (commit `bf89990`, PM-verified — faithfully mirrors the frontend's stream/window/ladder resolver) — **deploy-to-production status unconfirmed, see Log 2026-07-21e, this is now the single most important open question**
...
- **2026-07-21d — PM response to the off-cadence report (PR #353, gate overrides + Phase 2/3 shipped).**
  ...
  **CORRECTION, same day**: checked the actual repo before finalizing this concern — `get_health_stats` WAS already made source-aware, commit `bf89990`, landed right after the poller itself and before the CORS/error-surfacing/down-scoping fixes. Read it in full: faithfully mirrors the frontend's stream/window/ladder logic (same tier rules, same per-metric ladder assignment for all 6 metrics it covers, same richest-stream tie-break for same-tier duplicates). Withdrawing the "not done" framing below — it wasn't dropped, I just hadn't checked before responding last time. One point stands regardless (see 2026-07-21e).
...
- **2026-07-21e — PHASE 3 LIVE. First real Fitbit pull succeeded, DB-verified by the coordinator (step_count 672, hourly HR Min/Avg/Max, hourly active_energy, `google_health_sync_state.last_success_at` set, `last_error` null). PM verified the rest independently:**
  - **Resolution math checked by hand, not just accepted**: apple 2,959 + fitbit 672, 1 overlapping hour, reported result ≈3,099. `2,959 + 672 − 3,099 = 532` = exactly the losing stream's dropped contribution for that one hour; `3,099 − 2,959 = 140` = fitbit's genuine non-overlapping-hour contribution (`672 − 532 = 140`, consistent). Internally coherent — real evidence the CARDINAL RULE union-not-sum behavior is correct on real dual-source data, day one.
  - **Token verification ask — satisfied**: confirmed via my own read that `google-health-sync` mints from `user_calendar_tokens`'s live row (no test-token path exists in the code at all), and the coordinator independently confirmed via Google's own `tokeninfo` endpoint that the token carries all 5 scopes.
  - **Three fix PRs read in full, all correctly implemented**: `#354` CORS — `corsHeaders()` added matching `calendar-oauth`'s exact allow-list pattern, applied to OPTIONS/405/all JSON responses (confirmed the ORIGINAL file I reviewed genuinely lacked this — real bug, real fix). `#355` error-body surfacing — `FitbitSyncButton.tsx` now reads `error.context` as a `Response`, parses its real JSON body, and surfaces specific Turkish guidance for both `reconnect_required` and the disallowed-scope case specifically — better than a generic fix, tailored to the actual failure modes hit. `#356` down-scoping — `HEALTH_SCOPES` is the exact 3 correct scope URLs from the design doc §7, joined space-delimited, passed as `scope` on the refresh request; comment correctly documents that `calendar-token` is intentionally left alone since Calendar's API tolerates the wider token. The "vindicates half" framing is fair and appropriately measured — scope purity mattering was the real worry, client-level separation vs. token-level down-scoping is just two different mechanisms for the same underlying need, and down-scoping is the more elegant one given the user's unified-consent decision.
  - **`get_health_stats` — checked myself, not left as an open question this time**: confirmed shipped (commit `bf89990`, landed immediately after the poller and before the three fix PRs) and read it in full — faithful port of the frontend's stream/window/ladder resolver for all 6 metrics it covers. **The one thing that still needs a direct answer: has `ai-proxy` actually been REDEPLOYED with this code?** Edge Function deploys are manual per CLAUDE.md — code sitting correct in the repo doesn't mean it's the version currently running. Given real `fitbit` rows already exist in production (this report's own evidence), if the deployed `ai-proxy` predates commit `bf89990`, the AI is giving double-counted health answers RIGHT NOW, not hypothetically. **This is the single most important thing to confirm before the user leaves** — please state explicitly, in the next report, whether `ai-proxy` was redeployed and when, relative to `bf89990`.
  - **Still open from 2026-07-21d, not addressed in this report**: whether Calendar's own working feature (create/view/sync an event) was clicked and confirmed after the unified re-consent. Low urgency (none of the three fix PRs touched `calendar-token`), but still unconfirmed — mention it once more, then drop it if the next report doesn't need to touch Calendar again.
  - Everything else (cron as the last manual step, tonight's sleep resolving the §11 shape questions, v1.1 metrics as a follow-up PR, 28 Jul as a formality) — agreed, no changes.
