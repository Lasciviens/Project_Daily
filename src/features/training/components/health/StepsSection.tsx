...
export function StepsSection() {
  ...
  const { data: stepPoints = [], isLoading: stepsLoading } = useHealthMetricSeries('step_count', today, today)
  const { data: distPoints = [] } = useHealthMetricSeries('walking_running_distance', today, today)

  const steps = computeDailySeries('step_count', stepPoints)[0]?.value ?? 0
  const distanceKm = computeDailySeries('walking_running_distance', distPoints)[0]?.value ?? 0
  const hourly = computeHourlyBuckets('step_count', stepPoints)
