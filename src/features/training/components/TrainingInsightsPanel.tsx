...
import {
  sortFindings, computeConsistencyFindings, computeVolumeFindings, computeMuscleFindings,
        ...
        <ul className="flex flex-col gap-2">
          {findings.map(f => (
            <li key={f.id} className="text-xs text-ink-700 flex items-start gap-2">
              <span aria-hidden className="shrink-0 mt-0.5">{f.positive === true ? '✅' : f.positive === false ? '⚠️' : 'ℹ️'}</span>
              <span>
