...
export function TrainingPage() {
  ...
  return (
    <PageContainer width="full">
      {/* The shell header already names the page on phones. */}
      <PageHeader title={<span className="hidden sm:inline">Training</span>}>
        <div className="flex items-center gap-2">
          <div role="tablist" aria-label="Training sections" className="scroll-x flex min-w-0 flex-1 gap-1">
            {TABS.map(t => (
              <button
                key={t.id}
                type="button"
          ...
          {tab === 'hevy' && <span className="shrink-0"><HevySyncButton /></span>}
        </div>
      </PageHeader>
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between lg:gap-6">
        <div className={`w-full min-w-0 lg:max-w-4xl ${wide ? '2xl:max-w-none 2xl:flex-1' : ''}`}>
          {tab === 'hevy'   && <HevyTab />}
