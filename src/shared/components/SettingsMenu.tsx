    ...
    <div role="radiogroup" aria-label="Accent colour" className="flex flex-wrap gap-1">
      {(Object.entries(ACCENTS) as [AccentName, (typeof ACCENTS)[AccentName]][]).map(([name, preset]) => {
        ...
        return (
          ...
          >
            <span
              className={cx('grid h-6 w-6 place-items-center rounded-full ring-offset-2 ring-offset-surface', on && 'ring-2 ring-line-strong')}
  ...
  return (
    <>
      ...
      >
        ...
        <span className="flex-1">{push.busy ? 'Working…' : push.enabled ? 'Turn off notifications' : 'Turn on notifications'}</span>
      </button>
