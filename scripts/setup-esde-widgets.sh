...
for script in push-esde-library.py sync-esde.py; do
...
cp "$source_dir/push-esde-library.py" "$source_dir/sync-esde.py" "$app_dir/"
...
SH
  printf 'python "$HOME/.local/share/esde-sync/sync-esde.py" %s\n' "$option" >> "$HOME/.shortcuts/$filename"
  cat >> "$HOME/.shortcuts/$filename" <<'SH'
result=$?
if [ "$result" -ne 0 ]; then
...
create_widget 'Kapaklari-Guncelle' '--covers-only'
create_widget 'Kutuphane-Kontrol' '--dry-run'
python "$app_dir/sync-esde.py" --dry-run
